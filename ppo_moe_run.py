...
from algo.ppo import HierarchicalActorCritic 
import os
def get_args():
    ...
    parser.add_argument('--time_eps',default=1920,type=int,help="interaction steps of each episode")
    ...
    parser.add_argument('--actor_lr',default=5e-5,type=float,help="learning rate of actor")
    ...
    parser.add_argument('--entropy_coef',default=0.001,type=float,help="entropy regularization coefficient")
...
def train(cfg, env, hac: HierarchicalActorCritic,model_dir=None):
    ...
    for i_ep in range(cfg.train_eps):
        ...
        if model_dir is not None:
            if ep_reward > best_reward:
                ...
                try:
                    torch.save(hac.state_dict(), model_dir+"hac_model.pth")
                    print(f"🎉 模型成功保存！文件大小: {os.path.getsize(model_dir):.2f}")
...
if __name__ == "__main__":
    ...
    if cfg.env_name == 'standard':
        if cfg.season == 'summer':
            model_file = "/home/chenyang2/flexible_consumption/outputs/router_rule/ppo/summer20251128-100014/models/hac_model.pth"
        else:
            model_file = "/home/chenyang2/flexible_consumption/outputs/router_rule/ppo/winter20251128-144701/models/hac_model.pth"
        try:
            if not os.path.exists(model_file):
                raise FileNotFoundError(f"模型文件未找到: {model_file}")
            state_dict = torch.load(model_file, map_location=cfg.device)
            hac.load_state_dict(state_dict)
            hac.to(cfg.device)
        except Exception as e:
            print(f"加载 HAC 模型失败: {e}")
            raise
    ...
    res_dic = train(cfg, env, hac,model_dir=model_path)
    ...
    # try:
    #     torch.save(hac.state_dict(), model_path+"hac_model.pth")
    #     print(f"🎉 模型成功保存！文件大小: {os.path.getsize(model_path):.2f}")
