...
class FixDemandRuleEnv(object):
    def __init__(self,
                    seed=42,
        ):
        super(FixDemandRuleEnv, self).__init__()
        # 使用Python默认的float64类型
        self.max_ES_capacity = 2000.0  # 300kwh 1200kw`15min
    ...
    def process_ev_arrivals_to_info(self, time_slot: int):
        ...
        for i in range (len(self.EV_info)):
            if self.EV_info[i][3]:
                self.fundamental[i] = max(0, 0.95*self.EVbattery-self.EV_info[i][2]) / self.EV_info[i][0]
    ...
    def calculate_powerpurchasefee(self,power,t):
        power_cost= (self.pricepuchase[t]+0.03274+0.0285)*power/4
        return power_cost
    ...
    # def clip_action (env, consumptionflag, action,t):
        action = action.astype(float)
        assert env.ES_capacity >= 0
        assert env.ES_capacity <= env.max_ES_capacity
        assert sum(env.fundamental) >= 0
        penalty=0.0
        #power_input 给用户端最终的电， PGS 电网给储能， PG 电网给用户， power_discharge, power_charge
        if consumptionflag > 0:

            action[0] = (action[0] + 1) / 2 * env.ES_rateddischare  # 电池放电功率
            ESSoutputcapacity= min(env.ES_capacity - 0.1*env.max_ES_capacity, env.ES_rateddischare)
            action[1] = env.log_curve_mapping(action[1])  # 电网功率
            ...
            demand  = env.household_ele[t]+sum(env.fundamental)
            power_input = env.PV_GEN[t] + env.WT_GEN[t] +PG + power_discharge
            
            if power_input < demand: 
                action[1] = demand - (env.PV_GEN[t] + env.WT_GEN[t] + power_discharge)
                power_input = demand
                PG=action[1]           
                
            if power_input > env.household_ele[t] + env.maxEVpower*env.available_EV:
                #超过消纳能力，要把买的点和放的电减少。
                PG = env.household_ele[t] + env.maxEVpower*env.available_EV - (env.PV_GEN[t] + env.WT_GEN[t] + power_discharge)
                
                if PG < 0:
                    action[1]-=PG
                    PG=0
                    power_discharge=action[0]
                power_input= env.household_ele[t] + env.maxEVpower*env.available_EV
               
            return PG, power_input, power_discharge,penalty

        else:
            if env.PV_F[t] + env.WT_F[t] >= env.household_ele[t] + env.maxEVpower * env.available_EV + max(0,0.9*env.max_ES_capacity - env.ES_capacity):
                print("必须要发生弃电",t%96)
                power_storage=0.9*env.max_ES_capacity - env.ES_capacity
                power_input=env.household_ele[t] + env.maxEVpower * env.available_EV
                PG=0
                return power_input, power_storage, PG

            action[0] = (action[0] + 1) / 2
            PVS = env.PV_GEN[t] * action[0]  # 储存的PV功率
            WTS = env.WT_GEN[t] * action[0]
            demand = env.household_ele[t] + sum(env.fundamental)

            action[1] = env.log_curve_mapping(action[1])  #电网向储能
            action[2] = env.log_curve_mapping(action[2])  #电网向用户 
            power_storage = PVS + WTS + action[1]
            power_input = env.PV_GEN[t] - PVS+ env.WT_GEN[t]-WTS+action[2]
            PG = action[1] +action[2]  # 电网    

            storage_capability=min(0.9*env.max_ES_capacity-env.ES_capacity,env.ES_ratedcharge)
            ...
            if power_storage > storage_capability:
                ...
                if power_storage > storage_capability:  #需要减少储能比例
                    ...
                    power_input = env.PV_GEN[t]+ env.WT_GEN[t] - storage_capability +action[2]
            ...
            maxdemand=env.household_ele[t] + env.maxEVpower*env.available_EV
            ...
            return power_input, power_storage, PG,penalty
    ...
    def env_knowledge(self,t,season):
        ...
        if season=='summer':
            ...
            if self.pricepuchase[t]< 0.025:
                return 1
            else:
                return 2

    def step(self, t, consumptionflag,power_input, ess_change, PG):
        
        if consumptionflag==1:
            power_discharge=ess_change
        else:       
            power_storage=ess_change
        penalty=0.0
        ...
        #         PG=0
        #         if power_discharge > self.ES_capacity - 0.1*self.max_ES_capacity or power_discharge>self.ES_rateddischare:
        #             power_discharge = min(self.ES_capacity - 0.1*self.max_ES_capacity, self.ES_rateddischare)
        ...
        if consumptionflag==1: 
            assert self.ES_capacity >= 0.1*self.max_ES_capacity-1e-6
            assert PG >= 0 -1e-6
            assert power_discharge >= 0-1e-6
            chargeratio=1
        
            if self.available_EV>0: 
                if power_input - self.household_ele[t] - sum(self.fundamental) >0:
                    consumptionpower = (power_input - self.household_ele[t] - sum(self.fundamental)) / self.available_EV
                    
                else:
                    consumptionpower=0
                    if power_input - self.household_ele[t]>0:
                        chargepower= power_input- self.household_ele[t]
                        chargeratio = chargepower / sum(self.fundamental)
                    else:
                        chargeratio=0
                        penalty=power_input-self.household_ele[t]

                for i in active_cars:
                        self.EV_info[i][2] = self.EV_info[i][2] + chargeratio* self.fundamental[i] + consumptionpower
                        self.EV_info[i][0] = self.EV_info[i][0] - 1
            else:
                if power_input < self.household_ele[t]:
                    penalty= power_input - self.household_ele[t]
            ...
            battery_cost = self.calculate_battery_cost(power_discharge)
            PG_cost=self.calculate_powerpurchasefee(PG,t)
            reward = - PG_cost - battery_cost - self.PV_GEN[t] * self.PVprice - self.WT_GEN[t] * self.WTprice + 0.1*penalty
        else:
            # if self.ES_capacity > 0.9*self.max_ES_capacity:
                # print("错误，储能满了还进行充电")
                # print(self.ES_capacity,0.9*self.max_ES_capacity)
            assert self.ES_capacity <= 0.9*self.max_ES_capacity + 1e-3
            # 只对在站内且可充电的车辆分配充电功率
            assert PG >= 0- 1e-6
            assert power_storage >= 0- 1e-6
            chargeratio=1
            ...
            battery_cost = self.calculate_battery_cost(power_storage)
            PG_cost=self.calculate_powerpurchasefee(PG,t)   
            reward = - battery_cost - PG_cost - self.PV_GEN[t] * self.PVprice - self.WT_GEN[t] * self.WTprice + 0.1* penalty
        ...
        for i in range(50):
            ...
            if self.EV_info[i][3]:
                ...
                if duration_slots == 0 or battery_units >= 0.95*self.EVbattery:  # 停车结束或已充满就清除这辆车
                    
                    self.EV_info[i][3] = False
                    self.EV_info[i][2] -= self.EV_info[i][1] * self.eleconsumption_km #更新为下一次来充电的时候的电量
                    if self.EV_info[i][2] < 0:
                        reward += 0.1* self.EV_info[i][2]  # 电量为负，增加惩罚
                        penalty += self.EV_info[i][2]
                        self.EV_info[i][2] = 0.0
                    self.available_EV -= 1
        ...
        return next_state, reward, done ,price_f, price_c,purchase_fee,energyfee,penalty